            ...
            for j in range(0, len(classes)):
                if (j >= i):
                    if (classes[i].get_meetingTime() == classes[j].get_meetingTime() and classes[i].get_course().get_id() != classes[j].get_course().get_id()):
                        ...
                        if (classes[i].get_room() == classes[j].get_room() and classes[i].get_room().get_Capacity()< classes[j].get_course().get_Numgrstudents()):
...
class DisplayMgr:
    ...
    def print_schedule_by_student(self, schedule, student_id):
        ...
        table = PrettyTable()
        table.field_names = ["Class #", "Course (Id)", "Room (Id)", "Professor", "Meeting Time","Number of Students"]
        ...
        print(table)
        y=table.get_string(fields=["Course (Id)","Number of Students"])
        print(y)
    ...
    def print_schedule_by_professor(self, schedule):
        ...
        for professor_id in professors:
            ...
            table1 = PrettyTable()
            table1.field_names = ["Class #", "Course (Id)", "Room (Id)", "Meeting Time"]
    ...
    def print_capacity():
        table6=prettytable.PrettyTable(["Room #","Capacity"])
        rooms=data.get_rooms()
        for i in range(0,len(rooms)):
            table6.add_row([str(rooms[i].get_name()),str(rooms[i].get_Capacity())])
        print(table6)
